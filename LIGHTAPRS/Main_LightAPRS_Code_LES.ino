//The GT LES APRS Transmit and Control Software
...
//****************************************************************************
char  CallSign[7]="KM4DE"; //DO NOT FORGET TO CHANGE YOUR CALLSIGN
...
//char comment[50] = "GT LES Testing"; // Max 50 char
char StatusMessage[50] = "GT LES Testing"; 
//*****************************************************************************
...
float DraHighVolt=4.0;    // min Volts for radio module (DRA818V) to transmit (TX) 1 Watt, below this transmit 0.5 Watt. You don't need 1 watt on a balloon. Do not change this.
...
void aprs_msg_callback(struct AX25Msg *msg) {
  Serial.println(F("CALLBACK  CALLBACK  CALLBACK"));
...
void updateTelemetry() {
  ...
  telemetry_buff[16] = ' ';
  sprintf(telemetry_buff + 17, "%04d", TxCount);
  telemetry_buff[21] = 'T';
  telemetry_buff[22] = 'x';
  telemetry_buff[23] = 'C';
  telemetry_buff[24] = ' '; float tempC = bmp.readTemperature();//-21.4;//
  dtostrf(tempC, 6, 2, telemetry_buff + 25);
  telemetry_buff[31] = 'C';
  telemetry_buff[32] = ' '; float pressure = bmp.readPressure() / 100.0; //Pa to hPa
  dtostrf(pressure, 7, 2, telemetry_buff + 33);
  telemetry_buff[40] = 'h';
  telemetry_buff[41] = 'P';
  telemetry_buff[42] = 'a';
  telemetry_buff[43] = ' ';
  dtostrf(readBatt(), 5, 2, telemetry_buff + 44);
  telemetry_buff[49] = 'V';
  telemetry_buff[50] = ' ';
  sprintf(telemetry_buff + 51, "%02d", gps.satellites.isValid() ? (int)gps.satellites.value() : 0);
  telemetry_buff[53] = 'S';
  telemetry_buff[54] = ' ';
  //Add data to telemetry here like cutdown status
  //sprintf(telemetry_buff + 55, "%s", comment);
